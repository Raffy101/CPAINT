...
void fill(Point remplir, int ancienneCouleur , int couleur);
...
int main(int argc, char *argv[])
{
	...
	while(1)
	{
		...
		/*if((clic.x >=30 && clic.x <= 59) && (clic.y >=180 && clic.y<= 209))
		{
			afficher_image("images/pot_de_peinture_allumer.bmp", pot);
			Point milieu = attendre_clic();
			fill(milieu, blanc ,Couleur_def);
		}*/
...
void Init()
{
	...
	afficher_image("images/pot_de_peinture.bmp", pot);
	actualiser();
...
/*void fill(Point remplir, int ancienneCouleur , int couleur)
	{
		if (couleur_point(remplir) != ancienneCouleur)
			return;
			
		changer_pixel(remplir, couleur);
		actualiser();
		remplir.x = remplir.y + 2;
		fill(remplir, ancienneCouleur, couleur);

		remplir.x = remplir.x - 1;
		fill(remplir, ancienneCouleur, couleur);
		...
		while(clic_gauche.x != -1 && clic_gauche.y != -1)
			{
				...
				{	
					...
					actualiser();
					//dessiner_ligne(centre,clic_avant, blanc);	
...
int choix_taille(Point *clic)
{
	...
	clic_gauche = clic_a_eu_lieu();	
	printf("clic_gauche.x = %d clic_gauche.y = %d\n", clic_gauche.x, clic_gauche.y);	
